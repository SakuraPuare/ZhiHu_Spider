...
class Topic(Tables):
    name = 'topic'
    keys = ['uid', 'name', 'introduction', 'follow_count', 'question_count', 'avatar_url', 'best_answers_count',
            'topic_id']
    types = [
        ('id', 'bigint', 'auto_increment', 'PRIMARY KEY'),
        ('uid', 'bigint', "comment '话题id'"),
        ('name', 'varchar(64)', "comment '话题标题'"),
        ('introduction', 'text', "comment '简介'"),
        ('follow_count', 'bigint', "comment '关注数量'"),
        ('question_count', 'bigint', "comment '问题数量'"),
        ('avatar_url', 'varchar(128)'),
        ('best_answers_count', 'bigint'),
        ('topic_id', 'bigint'),
    ]
    ...
    def tolist(self, data: Union[dict, list[dict]]) -> list:
        ...
        for i in data:
            ...
            follow_count = i.get('follow_count', -1)
            question_count = i.get('question_count', -1)
            avatar_url = i.get('avatar_url', '')
            best_answers_count = i.get('best_answers_count', -1)
            topic_id = i.get('topic_id', '')
            result.add(
                (uid, name, introduction, follow_count, question_count, avatar_url, best_answers_count, topic_id))
        return list(result)
...
class Answer(Tables):
    ...
    keys = ['uid', 'question_id', 'author_id', 'created_time', 'updated_time', 'voteup_count', 'favlists_count',
            'comment_count', 'url', 'content', 'is_labeled']
    types = [
        ('id', 'bigint', 'auto_increment', 'primary key'),
...
class Comment(Tables):
    name = 'comment'
    keys = ['uid', 'author_id', 'content', 'created_time', 'vote_count', 'reply_to_author', 'url', 'address_text',
            'from_where']
    types = [
        ('id', 'bigint', 'auto_increment', 'primary key'),
    ...
    def tolist(self, data: Union[list[dict], dict]) -> list:
        ...
        for comment in data:
            ...
            from_where = 'answer'
            result.add(
                (uid, author_id, content, created_time, vote_count, reply_to_author, url, address_text, from_where))
        return list(result)
